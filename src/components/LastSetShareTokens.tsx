...
import { Copy, Plus, Trash2, KeyRound, Clock, Lock } from "lucide-react";
...
export function LastSetShareTokens({ profileId, username }: Props) {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from("lastset_share_tokens" as never)
      .select("*")
  ...
  function shareUrl(token: string) {
    return `${window.location.origin}/l/${username}?token=${token}`;
  }
  ...
  return (
    <div className="border border-border rounded-xl p-4 space-y-4 bg-card">
      <div className="flex items-center gap-2">
        ...
        <h4 className="font-semibold text-sm text-foreground">Private share links</h4>
        <Badge variant="outline" className="ml-auto text-xs">{tokens.length}</Badge>
      </div>
      <p className="text-xs text-muted-foreground">
        Share your profile with specific people without making it public. Each link works even when your profile is private — and can be revoked anytime.
      </p>
      ...
      <div className="grid sm:grid-cols-[1fr_auto_auto] gap-2 items-end">
        <div>
          ...
          <Input
            ...
            placeholder="e.g. Recruiter access"
            maxLength={80}
            className="h-9"
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {tokens.map((t) => {
            ...
            return (
              <div key={t.id} className="border border-border rounded-md p-3 bg-background space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <KeyRound className="w-3.5 h-3.5 text-muted-foreground" />
                  <span className="font-medium text-sm text-foreground">{t.label || "Untitled link"}</span>
                  {expired ? (
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <div className="ml-auto flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => revoke(t.id)} aria-label="Revoke link">
