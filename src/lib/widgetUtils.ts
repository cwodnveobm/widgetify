...
Sent via ${contactBusinessName} Contact Form\`;
    ...
    case 'refer-a-friend':
      const referralUrl = ${JSON.stringify('${shareUrl || 'https://widgetify-two.vercel.app/'}')};
      const referralMessage = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};

      return `${baseStyles}
        <div id="widgetify-referral" class="widgetify-widget" onclick="toggleWidgetifyReferral()" aria-label="Refer a friend">
          <svg width="30" height="30" viewBox="0 0 24 24" fill="white">
            <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7 0-.24-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92c0-1.61-1.31-2.92-2.92-2.92z"/>
          </svg>
        </div>

        <div id="widgetify-referral-popup" class="widgetify-popup" role="dialog" aria-labelledby="referral-title">
          <div class="widgetify-header">
            <h3 id="referral-title">Refer a Friend</h3>
            <button class="widgetify-close" onclick="toggleWidgetifyReferral()" aria-label="Close referral">×</button>
          </div>
          <div class="widgetify-content">
            <p style="margin:0 0 12px 0; font-size:14px; color:#374151;">${referralMessage}</p>
            <div style="display:flex; gap:8px; flex-wrap: wrap;">
              <button class="upi-gateway-button" onclick="widgetifyShareReferral()" style="flex:1; min-width:140px;">Share Now</button>
              <button class="upi-gateway-button" onclick="widgetifyCopyReferral()" style="flex:1; min-width:140px; background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%);">Copy Message</button>
            </div>
            <div style="display:flex; gap:10px; margin-top:12px; flex-wrap: wrap;">
              <button onclick="openWhatsAppReferral()" style="padding:10px 12px; border-radius:8px; background:#25D366; color:white; border:none; cursor:pointer;">WhatsApp</button>
              <button onclick="openEmailReferral()" style="padding:10px 12px; border-radius:8px; background:#2563eb; color:white; border:none; cursor:pointer;">Email</button>
              <button onclick="openTwitterReferral()" style="padding:10px 12px; border-radius:8px; background:#1DA1F2; color:white; border:none; cursor:pointer;">Twitter</button>
            </div>
          </div>
          <div class="widgetify-watermark">
            <a href="https://widgetify-two.vercel.app" target="_blank">Powered by Widgetify</a>
        ...
        <script>
          function toggleWidgetifyReferral() {
            const popup = document.getElementById('widgetify-referral-popup');
            popup.classList.toggle('show');
          }
          function widgetifyShareReferral() {
            var msg = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};
            var link = ${JSON.stringify(shareUrl || 'https://widgetify-two.vercel.app/')};
            if (navigator.share) {
              navigator.share({ title: 'Widgetify', text: msg, url: link });
            } else {
              window.open('https://wa.me/?text=' + encodeURIComponent(msg), '_blank');
            }
          }
          function widgetifyCopyReferral() {
            var msg = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};
          ...
          function openWhatsAppReferral() {
            var msg = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};
            window.open('https://wa.me/?text=' + encodeURIComponent(msg), '_blank');
          }
          function openEmailReferral() {
            var msg = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};
            window.location.href = 'mailto:?subject=' + encodeURIComponent('Check this out') + '&body=' + encodeURIComponent(msg);
          }
          function openTwitterReferral() {
            var msg = ${JSON.stringify(shareText || `Check out Widgetify - build chat widgets in seconds: ${shareUrl || 'https://widgetify-two.vercel.app/'}`)};
